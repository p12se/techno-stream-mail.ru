...
func IntSliceToString(inputSlice []int) string {
	var s string
...
func GetMapValuesSortedByKey(m map[int]string) []string {
	...
	for key := range m {
		keysOfMap = append(keysOfMap, key)
	}

	sort.Ints(keysOfMap)
	sorted := make([]string, 0, len(keysOfMap))

	for _, k := range keysOfMap {
